...
class SyncGoogleMetricsAction
{
    ...
    public function execute(User $user, ?string $propertyId = null): void
    {
        ...
        try {
            $selectedProp = MetricsCache::where('user_id', $user->id)
                ->where('provider', 'google')
                ->where('metric_key', 'ga4_selected_property')
                ->first();
                
            $propId = $propertyId ?? $selectedProp?->metric_value;

            if (!$propId) {
                // Si nunca seleccionó, agarramos la primera y la cacheamos como nombre
                $propData = $this->googleAnalyticsService->getFirstPropertyId($account);
                $propId = $propData['id'] ?? null;
                
                if (isset($propData['name'])) {
                    MetricsCache::updateOrCreate(
                        ['user_id' => $user->id, 'provider' => 'google', 'metric_key' => 'ga4_property_name'],
                        ['metric_value' => $propData['name'], 'recorded_at' => now()]
                    );
                }
            }
