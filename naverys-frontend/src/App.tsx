...
function App() {
  ...
  const [status, setStatus] = useState('Verificando session...')
  ...
  async function loadCurrentUser() {
    try {
      ...
      if (response.status === 401) { setUser(null); setStatus(''); return }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await apiFetch(isLogin ? '/api/auth/login' : '/api/auth/register', {
        method: 'POST',
        body: JSON.stringify(isLogin ? { email: form.email, password: form.password } : { name: form.name, ...form, password_confirmation: form.passwordConfirmation }),
      })
      const payload = (await response.json()) as any

      if (!response.ok) {
        if (payload?.errors) throw new Error(Object.values(payload.errors)[0]?.[0] as string)
